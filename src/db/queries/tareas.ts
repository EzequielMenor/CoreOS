...
// Suma días sobre un YYYY-MM-DD local. Nunca `new Date(hoy)` (parsea UTC y
// puede restar un día en zonas negativas). Parseo por partes + Date local.
function addDays(hoy: string, n: number): string {
...
export function normalizeDueDate(raw: string | null, hoy: string): string | null {
  ...
  if (s === 'mañana' || s === 'manana' || s === 'tomorrow') return addDays(hoy, 1);
  if (
    s === 'pasado mañana' ||
    s === 'pasado manana' ||
    s === 'day after tomorrow'
  ) {
    return addDays(hoy, 2);
  }
